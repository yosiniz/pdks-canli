...
router.get('/me', authenticateToken, (req, res) => {
  try {
    const db = getDb();
    const user = db.get('SELECT id, username, full_name, email, phone, role, profile_photo FROM users WHERE id = ?', [req.user.id]);
...
router.post('/change-password', authenticateToken, (req, res) => {
  try {
    ...
    db.run('UPDATE users SET password_hash = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?', [hash, req.user.id]);
