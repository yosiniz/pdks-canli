...
router.post('/check-in', authenticateToken, upload.single('selfie'), (req, res) => {
  try {
    const db = getDb();
    ...
    const location = db.get('SELECT * FROM locations WHERE id = ? AND qr_secret = ? AND is_active = 1', [qrInfo.locationId, qrInfo.secret]);
    ...
    const existing = db.get('SELECT * FROM attendance WHERE user_id = ? AND work_date = ? AND status = ?', [userId, today, 'checked_in']);
    ...
    const result = db.run('INSERT INTO attendance (user_id, location_id, work_date, check_in_time, check_in_lat, check_in_lng, check_in_selfie, is_gps_valid, status) VALUES (?,?,?,?,?,?,?,?,?)', [userId, location.id, today, now, lat, lng, selfiePath, 1, 'checked_in']);
    db.run('INSERT INTO gps_logs (attendance_id, user_id, latitude, longitude, is_within_zone, distance_meters) VALUES (?,?,?,?,?,?)', [result.lastInsertRowid, userId, lat, lng, 1, gpsCheck.distance]);
...
router.post('/check-out', authenticateToken, upload.single('selfie'), (req, res) => {
  try {
    const db = getDb();
    const { qr_data, latitude, longitude } = req.body;
    const userId = req.user.id;
    ...
    const gpsCheck = isWithinGeofence(lat, lng, location.latitude, location.longitude, location.radius_meters);
    const today = new Date().toISOString().split('T')[0];
    const attendance = db.get('SELECT * FROM attendance WHERE user_id = ? AND work_date = ? AND status = ?', [userId, today, 'checked_in']);
    if (!attendance) return res.status(400).json({ error: 'Aktif giris kaydi bulunamadi' });
    let selfiePath = null;
    if (req.file) selfiePath = '/uploads/selfies/' + today + '/' + req.file.filename;
    const now = new Date().toISOString();
    db.run('UPDATE attendance SET check_out_time = ?, check_out_lat = ?, check_out_lng = ?, check_out_selfie = ?, status = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?', [now, lat, lng, selfiePath, 'checked_out', attendance.id]);
...
router.post('/heartbeat', authenticateToken, (req, res) => {
  try {
    ...
    const lng = parseFloat(longitude);
    const today = new Date().toISOString().split('T')[0];
    const attendance = db.get('SELECT a.*, l.latitude as loc_lat, l.longitude as loc_lng, l.radius_meters FROM attendance a JOIN locations l ON a.location_id = l.id WHERE a.user_id = ? AND a.work_date = ? AND a.status = ?', [userId, today, 'checked_in']);
    if (!attendance) return res.status(400).json({ error: 'Aktif giris kaydi bulunamadi' });
...
router.get('/my-status', authenticateToken, (req, res) => {
  try {
    const db = getDb();
    const today = new Date().toISOString().split('T')[0];
    const attendance = db.get('SELECT a.*, l.name as location_name FROM attendance a JOIN locations l ON a.location_id = l.id WHERE a.user_id = ? AND a.work_date = ? ORDER BY a.id DESC LIMIT 1', [req.user.id, today]);
...
router.get('/my-history', authenticateToken, (req, res) => {
  try {
    ...
    const records = db.all('SELECT a.*, l.name as location_name FROM attendance a JOIN locations l ON a.location_id = l.id WHERE a.user_id = ? ORDER BY a.work_date DESC LIMIT ? OFFSET ?', [req.user.id, limit, offset]);
    const total = db.get('SELECT COUNT(*) as c FROM attendance WHERE user_id = ?', [req.user.id]);
